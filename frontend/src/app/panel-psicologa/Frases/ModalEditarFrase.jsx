...
export default function ModalEditarFrase({ frase, close, onUpdated }) {
  const API_URL = process.env.NEXT_PUBLIC_API_URL;

  const [texto, setTexto] = useState(frase.text || "");
  const [open, setOpen] = useState(false);
  const [severidad, setSeveridad] = useState(frase.severity || "alto");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/psychologist/phrases/${frase._id}`, {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          text: texto.trim(),
          severity: severidad,
          // mantenemos category y target originales si existen
          category: frase.category || "suicidio",
          target: frase.target || "self",
        }),
      });

      if (!res.ok) {
        const text = await res.text();
        console.error("Error actualizando frase:", text);
        return;
      }
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-40">
      <div className="bg-white w-[500px] rounded-xl p-6 shadow-lg relative">
        ...
        <div className="relative mt-2">
          ...
          >
            {opciones.find((o) => o.value === severidad)?.label || "Seleccionar"}
            <ChevronDown size={16} />
          </button>
